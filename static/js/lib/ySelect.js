(function($) {
        ...
        ySelect.prototype = {
            ...
            reload: function() {
                if (this.settings.showSearch) {
                    ...
                    + '" /><span class="fs-selectAll"><i class="mdi mdi-checkbox-blank-circle-outline" style = "color: black;font-size: 0.7rem;"></i></span></div>';
    ...
    $(document).on('click', '.fs-selectAll', function() {
        ...
        $(this).parent().next().find('.fs-optgroup-label').addClass('selected');
        $(this).addClass('selected');
    });
    $(document).on('click', '.fs-selectAll.selected', function() {
        $(this).parent().next().find('.fs-option.selected').click();
        $(this).parent().next().find('.fs-optgroup-label').removeClass('selected');
        $(this).removeClass('selected');
    });
    $(document).on('click', '.fs-optgroup-label', function() {
        $(this).parent().find('.fs-option.selected').click();
        $(this).parent().find('.fs-option').click();
        $(this).addClass('selected');
    });
    $(document).on('click', '.fs-optgroup-label.selected', function() {
        $(this).parent().find('.fs-option.selected').click();
        $(this).removeClass('selected');
    });
    $(document).on('click', '.fs-option.selected', function() {
        //console.log($(this).parent(),$(this).parent().parents('.fs-search'));
        $(this).parent().find('.fs-optgroup-label.selected').removeClass('selected');
        $(this).parent().parent().prev().find('.fs-selectAll.selected').removeClass('selected');
    });
    $(document).on('click', '.fs-option', function() {
        ...
        if ($wrap.hasClass('multiple')) {
            ...
            $(this).parent().find('.fs-optgroup-label').addClass('selected');
            $wrap.find('.fs-selectAll').each(function(i, el) {
                $(this).addClass('selected');
                $(this).find('.mdi.mdi-checkbox-blank-circle-outline').css('color','green');
                $(this).find('.mdi.mdi-checkbox-blank-circle-outline').removeClass('mdi-checkbox-blank-circle-outline');
                $(this).find('.mdi').addClass('mdi-checkbox-marked-circle-outline');
            });
            // $(this).parent().parent().prev().find('.fs-selectAll').addClass('selected');
            $wrap.find('.fs-option').each(function(i, el) {
                if(! $(this).hasClass('selected')){
                    $(this).parent().find('.fs-optgroup-label.selected').removeClass('selected');
                    $wrap.find('.fs-selectAll.selected').each(function(i, el) {
                        $(this).removeClass('selected');
                        $(this).find('.mdi.mdi-checkbox-marked-circle-outline').css('color','black');
                        $(this).find('.mdi.mdi-checkbox-marked-circle-outline').removeClass('mdi-checkbox-marked-circle-outline');
                        $(this).find('.mdi').addClass('mdi-checkbox-blank-circle-outline');
                    });
                }
            });
        }
    ...
    $.fn.ClearSelected = function(){
        ...
        $wrap.find('.fs-optgroup-label.selected').each(function(i, el) {
              $(this).removeClass('selected');
        });
        $wrap.find('.fs-selectAll.selected').each(function(i, el) {
              $(this).click();
